...
- [Docs — adoption guide](https://github.com/cooperjfecteau-cell/otlp-dotnet-alpine-musl-profiler/issues/26) and [structure](https://github.com/cooperjfecteau-cell/otlp-dotnet-alpine-musl-profiler/issues/16) — **the destination.** Two documents, because a customer needs both: `docs/GETTING_STARTED.md` is the happy path with the four UI-only steps flagged up front; `docs/LESSONS_LEARNED.md` is what saves them when it misbehaves, ending with the eight assumptions that were wrong. Repo also made portable — images now build amd64 as well as arm64, and tenant/cluster/credential identifiers are parameterised out of the manifests
- [Build — workflow trigger](https://github.com/cooperjfecteau-cell/otlp-dotnet-alpine-musl-profiler/issues/23) and [end-to-end verification](https://github.com/cooperjfecteau-cell/otlp-dotnet-alpine-musl-profiler/issues/25) — **the destination is reached.** One click on a Dynatrace workflow produced 5,540 eBPF records plus 1,503 CPU samples, 594 GCs and 97 contention groups from EventPipe, under one session, with no manual step. Reaches the private broker over EdgeConnect — which required a dedicated amd64 node group, since Dynatrace publishes no arm64 EdgeConnect image. The two halves arrive ~3 minutes apart and that is expected, not a fault
- [Research — musl native symbolization](https://github.com/cooperjfecteau-cell/otlp-dotnet-alpine-musl-profiler/issues/9) — **unavailable, not pending**, and deliberately not chased. Managed and kernel frames resolve at 100%; native ELF at 0%, because Alpine strips and Microsoft publishes no debuginfo for its musl runtime `.so` files. Shipped as `module+0xaddress` and coloured distinctly in the viewer. Dropping them would silently shorten stacks; a debuginfo sidecar would not help for the binaries that matter. Stated as a permanent platform limit in the docs
